...
func (m *MockNFTables) SyncPolicy(ctx context.Context, policy *datastore.Policy, operation nftables.SyncOperation, logger logr.Logger) error {
	// Track the operation and policy
	lastSyncedPolicy = policy
	lastSyncOperation = operation

	if operation == nftables.SyncOperationCreate {
		syncPolicyCreateCalled++
	} else if operation == nftables.SyncOperationDelete {
...
type TestNFTables struct {
}

func (t *TestNFTables) SyncPolicies(ctx context.Context, policies []*datastore.Policy) error {
	return nil
}
