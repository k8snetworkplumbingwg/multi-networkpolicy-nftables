...
func (n *NFTables) enforcePolicy(ctx context.Context, pod *corev1.Pod, interfaces []Interface, policy *datastore.Policy, logger logr.Logger) error {
	...
	err = createManagedInterfacesSet(tx, matchedInterfaces, hashName, policy.Namespace, policy.Name, logger)
	if err != nil {
		return fmt.Errorf("failed to create managed interfaces set: %w", err)
	}
...
func createCommonRules(tx *knftables.Transaction, commonRules *CommonRules, logger logr.Logger) {
	...
	combined := append(commonRules.CustomIPv4IngressRules, commonRules.CustomIPv6IngressRules...)
	...
	combined = append(commonRules.CustomIPv4EgressRules, commonRules.CustomIPv6EgressRules...)
	for _, rule := range combined {
...
func createManagedInterfacesSet(tx *knftables.Transaction, matchedInterfaces []Interface, hashName string, policyNamespace string, policyName string, logger logr.Logger) error {
