...
var _ = Describe("NFTables Simple Integration Tests", func() {
	...
	It("should handle deny-all policy", func() {
		...
		err = netNS.Do(func(netNS ns.NetNS) error {
			...
			err = verifyChainAndRules(nft, ctx, actualHashName, expectedPolicyRules)
			if err != nil {
				return err
			}
			return verifySetAndElements(nft, ctx, expectedSetElements)
		})
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("should handle comprehensive stacked policy", func() {
		...
		err = netNS.Do(func(netNS ns.NetNS) error {
			...
			err = verifyChainAndRules(nft, ctx, actualHashName, expectedRules)
			if err != nil {
				return err
			}
			return verifySetAndElements(nft, ctx, expectedSetElements)
	...
	It("should handle full livecycle", func() {
		...
		err = netNS.Do(func(netNS ns.NetNS) error {
			...
			err = verifyChainAndRules(nft, ctx, actualHashNameDeny, expectedPolicyRules)
			if err != nil {
				return err
			}
			err = verifySetAndElements(nft, ctx, expectedSetElements)
			if err != nil {
				return err
			}
			...
			err = verifyChainAndRules(nft, ctx, actualHashName, 14)
			if err == nil {
				return err
			}

			return verifyChainAndRules(nft, ctx, actualHashNameDeny, expectedPolicyRules)
		})
		Expect(err).NotTo(HaveOccurred())
	})
...
func verifyChainAndRules(nft knftables.Interface, ctx context.Context, hashName string, expectedRules int) error {
	// Get policy chain
...
func verifySetAndElements(nft knftables.Interface, ctx context.Context, expectedSetElements map[string]int) error {
	// Get all sets
